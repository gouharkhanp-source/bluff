...
export default function HomePage() {
  ...
  return (
    <SiteShell tone="#FFD23F">
      ...
      <section className="relative py-24 bg-white border-y-2 border-slate-900/10 overflow-hidden">
        <div className="container text-center">
          <Reveal>
            <h2 className="font-display text-4xl md:text-6xl text-exit-ink">BUILT FOR <span className="text-exit-red">YOUR CREW.</span></h2>
            <p className="mt-3 text-exit-ink/55">Whip it out anywhere the vibe needs a shake-up.</p>
          </Reveal>
            ...
            {OCCASIONS.map((o, i) => {
              ...
              return (
                <motion.div key={o.label} initial={{ opacity: 0, scale: 0.6, rotate: -8 }} whileInView={{ opacity: 1, scale: 1, rotate: 0 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 220, damping: 12, delay: i * 0.1 }}>
                  <div className="rounded-3xl border-2 border-slate-900/10 bg-white p-6 shadow-[4px_4px_0_0_#0f1e3d] hover-wiggle">
                    ...
                    <div className="mt-4 font-display text-lg text-exit-ink">{o.label}</div>
                  </div>
                </motion.div>
              )
            })}
              ...
              return (
                <motion.div key={s.small} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 200, damping: 14, delay: 0.2 + i * 0.12 }} className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <div className="font-display text-4xl md:text-5xl text-exit-ink leading-none">{s.big}</div>
                    <div className="label text-[10px] text-exit-ink/50">{s.small}</div>
                  </div>
                </motion.div>
              )
            })}
      ...
      <section className="relative py-24 md:py-28 overflow-hidden bg-[#fff7f7]">
        <div className="container relative">
          <Reveal>
            <h2 className="font-display text-4xl md:text-6xl text-center text-exit-ink leading-[1.08]">WIN THE CARDS.<br /><span className="text-exit-red text-glow">OR FACE THE CHALLENGE.</span></h2>
          </Reveal>
          <div className="mt-14 grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-5">
              <Reveal><div className="rounded-3xl border-2 border-slate-900/10 bg-white p-6"><div className="label text-xs text-exit-blue">2 PLAYERS</div><p className="mt-1 text-exit-ink/70">One player wins. The other faces the Challenge.</p></div></Reveal>
              <Reveal delay={0.1}><div className="rounded-3xl border-2 border-slate-900/10 bg-white p-6"><div className="label text-xs text-exit-blue">3–6 PLAYERS</div><p className="mt-1 text-exit-ink/70">The final two players face the Challenge.</p></div></Reveal>
            </div>
      ...
      {/* GAME MODES */}
      <section className="relative py-24 bg-[#eef7f6] border-y-2 border-slate-900/10">
        <div className="container">
          <Reveal><h2 className="font-display text-4xl md:text-6xl text-exit-ink">PICK YOUR <span className="text-exit-red">MODE.</span></h2></Reveal>
          <div className="mt-10 grid lg:grid-cols-[0.9fr_1.1fr] gap-8">
            <div className="flex flex-col gap-3">
              {GAME_MODES.map((m) => {
                const on = m.key === mode
                return (
                  <button key={m.key} onClick={() => setMode(m.key)} className={`text-left rounded-3xl border-2 p-5 transition-all ${on ? 'border-exit-red bg-white shadow-[4px_4px_0_0_#0f1e3d]' : 'border-slate-900/10 hover:border-slate-900/25 bg-white/60'}`}>
                    <span className="font-cond font-bold text-exit-ink">{m.title}</span>
                    <p className="text-xs text-exit-ink/55 mt-1">{m.short}</p>
                  </button>
                )
              })}
            </div>
            <motion.div key={mode} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} className="rounded-3xl border-2 border-slate-900/10 bg-white p-8 flex flex-col justify-center">
              <div className="label text-[10px] text-exit-red">SELECTED MODE</div>
              <h3 className="font-display text-4xl md:text-5xl mt-2 text-exit-ink">{active.title}</h3>
              <p className="mt-4 text-exit-ink/60 max-w-md">{active.desc}</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      {/* CTA */}
      <section className="relative py-24 md:py-28 text-center overflow-hidden bg-white">
        <FallingCards count={10} />
        <div className="container relative">
          <Reveal>
            <h2 className="font-display text-5xl md:text-7xl text-exit-ink">READY TO <span className="text-exit-red text-glow">EXIT?</span></h2>
            <p className="mt-4 text-exit-ink/60 max-w-lg mx-auto">Grab a deck, or jump into a free browser game right now.</p>
