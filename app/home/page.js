...
export default function HomePage() {
  ...
  return (
    <SiteShell tone="#14113a">
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                ...
                <Link href="/prebook" onClick={() => track(EVENTS.preBook, { from: 'hero' })} className="inline-flex items-center label text-xs px-7 py-4 rounded-full border-2 border-white/30 text-exit-cream hover:border-exit-cream hover:bg-white/[0.03] transition-colors">
                  PRE-BOOK THE DECK
                </Link>
              </div>
            </motion.div>
      ...
      {/* WIN RULES */}
      <section className="relative py-24 bg-exit-charcoal">
        <div className="container">
          <Reveal>
            <div className="flex items-end justify-between flex-wrap gap-4">
              <h2 className="font-display text-4xl md:text-6xl leading-[1.05] text-exit-cream">KNOW YOUR <span className="text-exit-red">WIN RULES.</span></h2>
              <p className="label text-[10px] text-exit-cream/40">THE GAME SELECTOR DECIDES</p>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
              ...
              {GAME_MODES.map((m) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left rounded-xl border p-5 transition-all ${on ? 'border-exit-red bg-exit-red/10' : 'border-white/10 hover:border-white/20 bg-exit-charcoal/40'}`}
                  >
        ...
        <div className="container relative">
          <Reveal>
            ...
            <div className="mt-8 flex flex-wrap gap-4 justify-center">
              ...
              <Link href="/editions" className="label text-xs px-8 py-4 rounded-full border-2 border-white/30 hover:border-exit-cream transition-colors">VIEW EDITIONS</Link>
            </div>
          </Reveal>
        </div>
      </section>
